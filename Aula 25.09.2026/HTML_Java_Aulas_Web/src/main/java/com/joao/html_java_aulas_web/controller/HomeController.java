...
import com.joao.html_java_aulas_web.model.Aluno;
import com.joao.html_java_aulas_web.model.Perfil;
import com.joao.html_java_aulas_web.model.Poupança;
...
public class HomeController {
	...
	private void addLimite(Model model) {
		LocalDate min=LocalDate.now().minusYears(120);
		LocalDate max=LocalDate.now().minusYears(5);
	...
	@PostMapping("/perfil")
	public String perfil(Model model,Perfil perfil) {
		addLimite(model);
		if(perfil.getCorfundo().equalsIgnoreCase(perfil.getCortexto())) {
			perfil.randomizar();
		}
	...
	public String academia(Model model,Aluno aluno) {
		double imc=aluno.calculaImc();
		String resposta=aluno.classificar(imc);
		model.addAttribute("aluno",aluno);
		DecimalFormat df=new DecimalFormat("00.0");
		model.addAttribute("imc",df.format(imc));
		model.addAttribute("situacao",resposta);
		return "academia";
	}
	
	@GetMapping("/poupança")
	public String poupança(Model model) {
		model.addAttribute("poupança",new Poupança());
		return "poupança";
	}
	@PostMapping("/poupança")
	public String poupança(Model model,Poupança poupança) {
		poupança.setTaxa(poupança.getTaxa()/100);
		DecimalFormat df=new DecimalFormat("#,###.00");
		double rendimento=poupança.calcularJuros();
		double saldo=poupança.getSaldo();
		System.out.println(rendimento);
		model.addAttribute("poupança",poupança);
		model.addAttribute("rendimento",df.format(rendimento));
		model.addAttribute("saldo",df.format(saldo));
		return "poupança";
	}
}
